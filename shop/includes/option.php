...
function get_option( $option, $default_value = false ) {
	...
	if ( is_scalar( $option ) ) {echo "<p>get_option LINE:".__LINE__."</p>"; 
		$option = trim( $option );
	}
	...
	$pre = apply_filters( "pre_option_{$option}", false, $option, $default_value );echo "<p>get_option LINE:".__LINE__."</p>"; 

	
	$pre = apply_filters( 'pre_option', $pre, $option, $default_value );echo "<p>get_option LINE:".__LINE__."</p>"; 

	if ( false !== $pre ) {
		return $pre;
	}
    echo "<p>get_option LINE:".__LINE__."</p>"; 
    ...
    echo "<p>get_option LINE:".__LINE__."</p>"; 
	if ( ! wp_installing() ) {echo "<p>get_option LINE:".__LINE__."</p>"; // load.php
		$alloptions = wp_load_alloptions();echo "<p>get_option LINE:".__LINE__."</p>"; 
		/*
...
function wp_load_alloptions( $force_cache = false ) {
	...
	if ( ! $alloptions ) {
        ...
        $debug_funct = __FUNCTION__;
        echo "<p>wp_load_alloptions LINE:".__LINE__."</p>";
        $sql = "SELECT option_name, option_value FROM $wpdb->options WHERE autoload IN ( '" . implode( "', '", esc_sql( wp_autoload_values_to_autoload() ) ) . "' )";
        echo "<p>wp_load_alloptions LINE:".__LINE__."</p>";
        echo "<p>wp_load_alloptions wpdb->options:".$wpdb->options."</p>";
        //echo "<p>wp_load_alloptions $sql:".esc_html($sql)."</p>";
        
		//$alloptions_db = $wpdb->get_results($sql);
		$num_row = $wpdb->query($sql);
		$alloptions_db = $wpdb->last_result;
        echo "<p>wp_load_alloptions LINE:".__LINE__."</p>";
        echo "<p>wp_load_alloptions num_row:".$num_row."</p>";
		if ( ! $alloptions_db ) {
		    ...
		    echo "<p>wp_load_alloptions LINE:".__LINE__."</p>";
		    //$guessurl = wp_guess_url();echo "<p>wp_load_alloptions LINE:".__LINE__."</p>"; //$url = WP_SITEURL;
		    $stylesheet = WP_DEFAULT_THEME;echo "<p>wp_load_alloptions LINE:".__LINE__."</p>"; //'twentytwentyfive' //default-constants.php//wp-settings.php
        	$template   = WP_DEFAULT_THEME;echo "<p>wp_load_alloptions LINE:".__LINE__."</p>"; //'twentytwentyfive'
        	//is_array( $wp_theme_directories )
        	//count( $wp_theme_directories )
        	//$theme_root = get_option( 'stylesheet_root' );
        	//$theme_root = get_option( 'template_root' );
        	//$theme_roots = get_theme_roots();
        	$theme      = wp_get_theme( WP_DEFAULT_THEME );echo "<p>wp_load_alloptions LINE:".__LINE__."</p>";//WP_Theme
	        if ( ! $theme->exists() ) {
            ...
            if ( ! empty( $insert ) ) {
                $sql =  "INSERT INTO $wpdb->options (option_name, option_value, autoload) VALUES " . $insert;
